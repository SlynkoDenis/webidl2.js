...
import { Class } from "./productions/class.js";
import { inheritance } from "./productions/container.js";
import { IntegerEnum } from "./productions/integer-enum.js";
...
function parseByTokens(tokeniser, options) {
  ...
  function enum_() {
    ...
    const baseType = inheritance(tokeniser);
    if (!baseType.inheritance) {
      // WebIDL String enum
      return Enum.parse(tokeniser, tokens);
    }
    Object.assign(tokens, baseType);
    return IntegerEnum.parse(tokeniser, tokens);
  }

  function class_(opts) {
    /** @type {Base["tokens"]} */
    const tokens = {};
    tokens.public = consume("public");
    tokens.base = consume("class");
    if (!tokens.base) {
      return;
    }
    return (
      Class.parse(tokeniser, tokens, { ...opts }) || error("Class has no proper body")
    );
  }
